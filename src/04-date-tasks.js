...
function timeSpanToString(startDate, endDate) {
  ...
  sec = sec > 9 ? sec : zero + sec;
  if (milSec < 100) {
    milSec = milSec > 9 ? zero + milSec : zero + zero + milSec;
  }
...
function angleBetweenClockHands(date) {
  ...
  const newHours = (hours * 60 + min) * (360 / (12 * 60));
  const newMin = min * (360 / 60);
  let angle = Math.abs(newHours - newMin);
  if (angle > 180) {
    angle = 360 - angle;
  }
